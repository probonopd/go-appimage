...
func AppDirDeploy(path string) {
	...
	determineELFsInDirTree(appdir.Path)
	...
	//TODO: Check error
	ldLinux, _ := deployInterpreter(appdir)
	...
	for i := 5; i <= 99; i++ {
		if containsString(allELFs, fmt.Sprintf("libQt%dCore.so.%d", i, i)) {
...
func deployInterpreter(appdir helpers.AppDir) (string, error) {
	...
	if options.standalone {
		...
		if err == nil {
			...
			determineELFsInDirTree(gconvs[0])
		}
...
func determineELFsInDirTree(pathToDirTreeToBeDeployed string) {
...
func getDeps(binaryOrLib string) error {
	...
	if helpers.IsDirectory(binaryOrLib) {
		return nil
	}
...
func handleQt(appdir helpers.AppDir, qtVersion int) {
	...
	if qtVersion >= 5 {
		...
		determineELFsInDirTree(qtPrfxpath + "/plugins/platforms/libqxcb.so")
		...
		for _, want := range wants {
			...
			if len(found) > 0 {
				determineELFsInDirTree(found[0])
			}
		}
		...
		for _, lib := range allELFs {
			if strings.HasSuffix(lib, fmt.Sprintf("libQt%dGui.so.%d", qtVersion, qtVersion)) {
				if helpers.Exists(qtPrfxpath + "/plugins/iconengines/") {
					determineELFsInDirTree(qtPrfxpath + "/plugins/iconengines/")
				} else {
				...
				if helpers.Exists(qtPrfxpath + "/plugins/imageformats/") {
					determineELFsInDirTree(qtPrfxpath + "/plugins/imageformats/")
				} else {
		...
		for _, lib := range allELFs {
			if strings.HasSuffix(lib, fmt.Sprintf("libQt%dGui.so.%d", qtVersion, qtVersion)) ||
				strings.HasSuffix(lib, fmt.Sprintf("libQt%dOpenGL.so.%d", qtVersion, qtVersion)) ||
				strings.HasSuffix(lib, fmt.Sprintf("libQt%dXcbQpa.so.%d", qtVersion, qtVersion)) ||
				strings.HasSuffix(lib, "libxcb-glx.so") {
				{
					determineELFsInDirTree(qtPrfxpath + "/plugins/xcbglintegrations/")
					break
				}
			}
		}
		...
		for _, lib := range allELFs {
			if strings.HasSuffix(lib, fmt.Sprintf("libQt%dPrintSupport.so.%d", qtVersion, qtVersion)) {
				determineELFsInDirTree(qtPrfxpath + "/plugins/printsupport/libcupsprintersupport.so")
				break
			}
		}
		...
		for _, lib := range allELFs {
			if strings.HasSuffix(lib, fmt.Sprintf("libQt%dNetwork.so.%d", qtVersion, qtVersion)) {
				determineELFsInDirTree(qtPrfxpath + "/plugins/bearer/")
				break
			}
		}
		...
		for _, lib := range allELFs {
			if strings.HasSuffix(lib, fmt.Sprintf("libQt%dSql.so.%d", qtVersion, qtVersion)) {
				determineELFsInDirTree(qtPrfxpath + "/plugins/sqldrivers/")
				break
			}
		}
		...
		for _, lib := range allELFs {
			if strings.HasSuffix(lib, fmt.Sprintf("libQt%dPositioning.so.%d", qtVersion, qtVersion)) {
				determineELFsInDirTree(qtPrfxpath + "/plugins/position/")
				break
			}
		}
		...
		for _, lib := range allELFs {
			if strings.HasSuffix(lib, fmt.Sprintf("libQt%dMultimedia.so.%d", qtVersion, qtVersion)) {
				determineELFsInDirTree(qtPrfxpath + "/plugins/mediaservice/")
				determineELFsInDirTree(qtPrfxpath + "/plugins/audio/")
				break
			}
		}
		...
		for _, lib := range allELFs {
			if strings.HasSuffix(lib, fmt.Sprintf("libQt%dWebEngineCore.so.%d", qtVersion, qtVersion)) {
		...
		for _, qmlImport := range qmlImports {
			...
			if qmlImport.Type == "module" && qmlImport.Path != "" {
				...
				path.Join("sss", "sss")
				determineELFsInDirTree(path.Join(appdir.Path, qmlImport.Path))
			}
		}
	}
}

func getQtPrfxpath(f *os.File) string {
