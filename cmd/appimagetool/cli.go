...
func main() {
	...
	err := helpers.CheckForNeededTools("file", "mksquashfs", "desktop-file-validate", "uploadtool", "patchelf", "desktop-file-validate", "patchelf") // "sh", "strings", "grep" no longer needed?; "curl" is needed for uploading only, "glib-compile-schemas" is needed in some cases only
	if err != nil {
		os.Exit(1)
	}
