...
func SignAppImage(path string, digest string) error {
	...
	_, err := crypto.NewKeyFromReader(io.Reader(pubkeyFileBuffer))
	if err != nil {
		fmt.Println("crypto.Key.ReadArmoredKeyRing error while reading public key:", err)
		return err
	}
	...
	privkey, err := crypto.NewKeyFromReader(io.Reader(privkeyFileBuffer))
	if err != nil {
		fmt.Println("crypto.Key.ReadArmoredKeyRing error while reading private key:", err)
		return err
	}
